...
function Load_From_Arrays!(mesh, dyn_data, data, model_type::Symbol)
    ...
    for (key, user_array) in data
        if key == :u || key == :v; continue; end # Handled later
        ...
        if haskey(var_map, key)
            ...
            if target_field isa AbstractArray
                 if size(target_field) == size(user_array)
                     ...
                     error("Dimension mismatch for :$key. Expected $(size(target_field)), got $(size(user_array))")
                 end
    ...
    if haskey(data, :u) && haskey(data, :v)
        ...
        Vor_Div_From_Grid_UV!(mesh, dyn_data.grid_u_c, dyn_data.grid_v_c, 
                              dyn_data.spe_vor_c, dyn_data.spe_div_c)
...
"""
function Dispatch_Standard_IC!(mesh, atmo_data, dyn_data, vert_coord, ic_name::Symbol, config)
    ...
    for k = 1:mesh.nd
        for j = 1:mesh.nθ
            for i = 1:mesh.nλ
                c_theta = cosθ[j]
                s_theta = sinθ[j]
                grid_u[i, j, k] = 25.0 * c_theta - 30.0 * c_theta^3 + 300.0 * s_theta^2 * c_theta^6
                grid_v[i, j, k] = 0.0
            end
        end
    end
    ...
    for k = 1:mesh.nd
        for j = 1:mesh.nθ
            for i = 1:mesh.nλ
                ...
                perturbation = (A / 2.0) * cosθ[j] * exp(-((θc[j] - θ0) / θw)^2) * cos(m * λc[i])
...
# ==============================================================================
function Init_Shallow_Water_Test!(mesh::Spectral_Spherical_Mesh, dyn_data::Dyn_Data, config::Model_Config)
    ...
    Vor_Div_From_Grid_UV!(mesh, dyn_data.grid_u_c, dyn_data.grid_v_c, dyn_data.spe_vor_c, dyn_data.spe_div_c)
    Trans_Spherical_To_Grid!(mesh, dyn_data.spe_vor_c, dyn_data.grid_vor)
    Trans_Spherical_To_Grid!(mesh, dyn_data.spe_div_c, dyn_data.grid_div)
