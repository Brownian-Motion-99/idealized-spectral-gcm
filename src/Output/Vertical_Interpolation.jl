...
function Compute_Pressure_Grid!(
    ...
    ak::Vector{Float64}, bk::Vector{Float64}, 
...
function Interpolate_Field!(
    ...
    @threads for j in 1:nθ
        for i in 1:nλ
            ...
            col_t = (strategy == :hydrostatic && !isnothing(t_3d)) ? view(t_3d, i, j, :) : nothing

            # Check direction (Standard: p[1] is Top, p[end] is Bottom)
            is_down = col_p[end] > col_p[1]

            for k_tgt in 1:n_plev
                ...
                if p_target > ps_true
                    ...
                    elseif strategy == :lapse_rate
                        ...
                        out_3d[i, j, k_tgt] = val_lowest_level * (p_target / p_lowest_level)^ALPHA
                    ...
                    elseif strategy == :hydrostatic
                        ...
                        if isnothing(col_t)
                            ...
                            out_3d[i, j, k_tgt] = val_lowest_level - (R_d * T_mean) * (lt - log(p_lowest_level))
                        end
                    else
                        out_3d[i, j, k_tgt] = val_lowest_level
                    end
