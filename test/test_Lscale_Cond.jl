...
@testset "Large-scale condensation signs, units, and heating scale" begin
    ...
    @test weak_heating.humidity_tendency ≈ full_heating.humidity_tendency
    @test weak_heating.temperature_tendency ≈ 0.25 .* full_heating.temperature_tendency
    @test weak_heating.precipitation ≈ full_heating.precipitation
    @test all(iszero, no_heating.temperature_tendency)
    @test no_heating.humidity_tendency ≈ full_heating.humidity_tendency
    @test no_heating.precipitation ≈ full_heating.precipitation
...
@testset "Large-scale condensation effective interval and prior adjustment" begin
    ...
    humidity = reshape(
        [
            0.95 * lscale_saturation_specific_humidity(atmo, temperature[k], p_full[k])
            for k = 1:nd
        ],
        nλ,
        nθ,
    ...
    prior_temperature_tendency = zeros(nλ, nθ, nd)
    prior_humidity_tendency = fill(2.0e-6, nλ, nθ, nd)
    temperature_tendency = zeros(nλ, nθ, nd)
    humidity_tendency = zeros(nλ, nθ, nd)
    liquid_water_content = zeros(nλ, nθ, nd)
    precipitation = zeros(nλ, nθ, 1)
    ...
    @test all(humidity_tendency .< 0.0)
