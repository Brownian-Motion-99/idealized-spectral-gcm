...
function velocity_profile(IC::String = "rigid_rotation")
    ...
    if IC == "rigid_rotation"
        ...
        for k = 1:nd
            ...
            for i = 1:nλ
                grid_vor0[i, :, k] =
                    2 * U / radius * (cos(β) * sinθ - sin(β) * cos(λc[i]) * cosθ)
            end
        end
    ...
    elseif IC == "random_2"
        ...
        for k = 1:nd
            for i = 1:nλ
                for j = 1:nθ
                    grid_u0[i, j, k] =
                        25 * cosθ[j] - 30 * cosθ[j]^3 + 300 * sinθ[j]^2 * cosθ[j]^6
                    grid_v0[i, j, k] = 0.0
                    grid_div0[i, j, k] = 0.0
                    grid_vor0[i, j, k] =
                        -(
                            -50 * sinθ[j] +
                            120 * cosθ[j]^2 * sinθ[j] +
                            300 * (2cosθ[j]^7 * sinθ[j] - 7cosθ[j]^5 * sinθ[j]^3)
                        ) / radius
                end
            end
        end
...
function test_derivative(IC::String = "rigid_rotation")
    ...
    nλ, nθ, nd, num_fourier, num_spherical =
        mesh.nλ, mesh.nθ, mesh.nd, mesh.num_fourier, mesh.num_spherical
...
function test_advection(IC::String = "rigid_rotation")
    ...
    for k = 1:nd
        for i = 1:nλ
            for j = 1:nθ
                grid_hs0[i, j, k] =
                    25 * cosθ[j] - 30 * cosθ[j]^3 + 300 * sinθ[j]^2 * cosθ[j]^6 * sin(λc[i])
                ...
                grid_∇hs0[i, j, 2, k] =
                    (
                        ...
                        1800 * sinθ[j]^3 * cosθ[j]^5 * sin(λc[i])
                    ) / radius
            end
        end
    end
